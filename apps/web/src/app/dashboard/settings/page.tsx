                  ...
                  <div className="grid gap-4">
                    ...
                    <div className="pt-4 mt-2 border-t" style={{ borderColor: "var(--border)" }}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--text-muted)" }}>Company / Project Name</label>
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 rounded-2xl bg-slate-900 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider mb-0.5 opacity-80">
                      Current Plan
                    </p>
                    ...
                    <p className="text-sm opacity-75 mt-1">
                      {profile.plan === "free" && profile.planLimit === 3000
                        ? "3,000 emails/month · 100 emails/day"
                        : profile.plan === "free" && profile.planLimit === 500
                        ? "500 emails/month · 100 emails/day"
                        : profile.plan === "event_level"
                        ? "10,000 emails/month · Event tracking"
                        : profile.plan === "starter"
                        ? "50,000 emails/month · Standard analytics"
                        : profile.plan === "growth"
                        ? "100,000 emails/month · Advanced tools"
                        : profile.plan === "premium"
                        ? "1,000,000 emails/month · Premium support"
                        : "Up to 100M+ emails · Dedicated IPs"}
                    </p>
                  </div>
                  <Zap size={32} className="opacity-80 md:hidden" />
                </div>

                {/* Quota Progress Bar (Monthly) */}
                <div className="mt-6">
                  <div className="flex justify-between text-sm font-medium mb-1.5 opacity-90">
                    <span>Monthly Usage</span>
                    <span>
                      {profile.monthlyEmailCount.toLocaleString()} /{" "}
                      {profile.planLimit >= 10000000 ? "Unlimited" : profile.planLimit.toLocaleString()}
                    </span>
                  </div>
                  <div className="w-full bg-white/20 rounded-full h-2.5 mb-1 overflow-hidden">
                    <div 
                      className="bg-white h-2.5 rounded-full transition-all duration-500 ease-out" 
                      style={{ width: `${Math.min(100, ((profile.monthlyEmailCount || 0) / (profile.planLimit || 1)) * 100)}%` }}
                    ></div>
                  </div>
                  <div className="text-xs opacity-75 mt-1 flex justify-between">
                    <span>Resets: {profile.billingPeriodStart ? new Date(new Date(profile.billingPeriodStart).setMonth(new Date(profile.billingPeriodStart).getMonth() + 1)).toLocaleDateString() : "Next month"}</span>
                  </div>
                </div>

                {/* Quota Progress Bar (Daily - if applicable) */}
                {profile.dailyLimit !== null && (
                  <div className="mt-4">
                    <div className="flex justify-between text-sm font-medium mb-1.5 opacity-90">
                      <span>Daily Usage</span>
                      <span>
                        {profile.dailyEmailCount.toLocaleString()} / {profile.dailyLimit.toLocaleString()}
                      </span>
                    </div>
                    <div className="w-full bg-white/20 rounded-full h-2.5 mb-1 overflow-hidden">
                      <div 
                        className="bg-white h-2.5 rounded-full transition-all duration-500 ease-out" 
                        style={{ width: `${Math.min(100, ((profile.dailyEmailCount || 0) / (profile.dailyLimit || 1)) * 100)}%` }}
                      ></div>
                    </div>
                    <div className="text-xs opacity-75 mt-1">Resets: Tomorrow</div>
                  </div>
                )}
              </div>
              {/* Plans Comparison */}
              <div className="relative">
                ...
                <div 
                  ref={plansScrollRef}
                  className="flex gap-5 overflow-x-auto pb-6 pt-2 snap-x scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
                >
                  {[
                    {
                      id: "free",
                      ...
                      period: "forever",
                      features: [
                        "Up to 3k emails (Month 1)",
                        "500 emails/month (Ongoing)",
                    ...
                    <div
                      key={plan.id}
                      className={`min-w-[260px] shrink-0 snap-start p-4 rounded-xl border-2 transition-all ${profile.plan === plan.id ? "border-indigo-400 bg-indigo-50" : (plan as any).popular ? "border-indigo-200 bg-white" : "border-gray-200 bg-white"}`}
                    >
                      {(plan as any).popular && (
                      ...
                      {profile.plan === plan.id ? (
                        <div className="text-xs text-center text-indigo-600 font-semibold py-2 rounded-lg bg-indigo-100">
                          ✓ Current Plan
                        </div>
                      ) : (
                        <button
                          className={`w-full py-2 rounded-lg text-xs font-semibold transition-all ${plan.id === "enterprise" ? "border-2 border-gray-300 text-gray-600 hover:border-indigo-400 hover:text-indigo-600" : "bg-indigo-600 text-white hover:bg-indigo-700"}`}
                        >
                          {plan.id === "enterprise"
                            ? "Contact Sales"
