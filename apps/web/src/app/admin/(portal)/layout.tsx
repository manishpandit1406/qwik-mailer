...
export default function AdminLayout({
  ...
  const [user, setUser] = useState<{name: string, email?: string, role?: string}>({ name: "Admin", email: "admin@qwikmailer.in", role: "admin" });
    ...
    const fetchMe = async () => {
      try {
        ...
        if (json.success && json.data) {
          ...
          setUser(prev => ({ ...prev, role: freshUser.role, name: freshUser.name || prev.name }));
      ...
      <header className="h-16 shrink-0 flex items-center justify-between px-4 lg:px-6 bg-white border-b border-gray-200 z-50">
        <div className="flex items-center gap-4">
          ...
          <Link href="/admin" className="flex items-center gap-2.5 font-bold">
            <div className="w-8 h-8 rounded-md flex items-center justify-center bg-gray-900 shadow-sm">
              <ShieldAlert size={16} className="text-white" />
            </div>
            <span className="text-black tracking-tight font-bold hidden sm:block">Admin Panel</span>
          </Link>
        </div>

        <div className="ml-auto flex items-center gap-4">
          <Link href="/dashboard" className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100 border border-gray-200 hover:bg-gray-200 transition-colors cursor-pointer">
            <ArrowLeft size={12} className="text-gray-500" />
            <span className="text-xs font-semibold text-gray-600">Back to Dashboard</span>
          </Link>
        ...
        >
          ...
          <div className="h-16 flex items-center px-4 border-b border-gray-100 shrink-0 lg:hidden">
            <Link href="/admin" className="flex items-center gap-2.5 font-bold">
              ...
              <span className="text-black tracking-tight font-bold whitespace-nowrap">Admin Panel</span>
            </Link>
            <button
              onClick={() => setMobileOpen(false)}
