...
export function requireFeature(feature: "webhooks" | "scheduling" | "prioritySupport") {
  return async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!limits || !limits.features[feature]) {
      return reply.code(402).send({ success: false, error: `Your current plan (${plan}) does not support ${feature}. Please upgrade.` });
    }
  };
}
...
export async function checkValidationQuota(req: FastifyRequest, reply: FastifyReply) {
  ...
  if (user.monthlyValidationCount + requestedCount > limits.validationsPerMonth) {
    return reply.code(402).send({ success: false, error: `Validation quota exceeded. Limit is ${limits.validationsPerMonth} per month on the ${plan} plan.` });
  }
}
...
export async function checkContactQuota(req: FastifyRequest, reply: FastifyReply) {
  ...
  const res = await db.execute(sql`SELECT COUNT(*) as count FROM contacts WHERE team_id IN (${sql.raw(userTeams.map(t => `'${t.id}'`).join(',') || "''")})`);
  ...
  if (currentContacts + requestedCount > limits.maxContacts) {
    return reply.code(402).send({ success: false, error: `Contact limit exceeded. Maximum ${limits.maxContacts} contacts allowed on the ${plan} plan.` });
  }
}
...
export async function checkTeamMemberLimit(req: FastifyRequest, reply: FastifyReply) {
  ...
  const res = await db.execute(sql`SELECT COUNT(*) as count FROM team_members WHERE team_id = ${teamId}`);
  const rows = (res as any).rows || res;
